    ...
    case response of
        ...
        BadStatus_ metadata body ->
            case Decode.decodeString errorDecoder body of
                ...
                Err _ ->
                    "Unexpected error : ( status => " ++ String.fromInt metadata.statusCode ++ " response => " ++ body ++ " )"
                    |> Failure
                    |> Ok
...
apiResponseDecoder call =
    case call of
        GetAll -> Decode.map TodosLoaded (Decode.list todoDecoder)
        Create -> Decode.map TodoCreated todoDecoder
        Update -> Decode.map TodoUpdated todoDecoder
        Delete id -> Decode.map TodoDeleted (Decode.succeed id)
